...
export const RenderOnlyBus = ({ routeSelected, showAdviseModal, assetsTypeRoutes }) => {
  ...
  const _getBuses = async () => {
    const busesResponse = await axios.get(`https://wsmio.siur.com.co:8083/apiMIO/jaxrs/busInfo/${routeSelected.name}`)
    for (const data of busesResponse.data) {
      const returnData = data.longitud.toString().slice(0, 3)
      const returnData2 = data.longitud.toString().slice(3)
      const returnDataY = data.latitud.toString().slice(0, 1)
      const returnDataY2 = data.latitud.toString().slice(1)
      const x = returnData + '.' + returnData2
      const y = returnDataY + '.' + returnDataY2
      data.longitud = parseFloat(x)
      data.latitud = parseFloat(y)
    }
    setBuses(busesResponse.data)
  }

  const validateIconBuses = (data) => {
    const { urlImageBusSuccess, urlImageBusError } = assetsTypeRoutes[data.line[0]]
  ...
  return buses.map((x, i) => (
    <View key={x.busNumber.toString()}>
      ...
      >
        <Text style={styles.textBus} >{x.line}</Text>
